...
async fn do_deploy<B: Borrow<M> + Clone, M: JsonRpcClient + Middleware>(pending_tx: PendingTransaction<'_, M>, abi: Abi, client: B) -> Result<(ContractInstance<B, M>, TransactionReceipt), ContractError<M>> {
    let receipt = pending_tx
        .confirmations(1)
        .await
        .map_err(|_| ContractError::ContractNotDeployed)?
